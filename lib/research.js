...
const DEFAULT_API_TYPE = process.env.OPENAI_API_TYPE || "responses_web_search";
const DEFAULT_CHAT_SEARCH_MODEL = process.env.OPENAI_CHAT_SEARCH_MODEL || "gpt-5-search-api";
const MAX_BODY_BYTES = 1024 * 1024;

const RESPONSE_MODEL_OPTIONS = [
  {
    id: "gpt-5.4-nano",
    label: "1. GPT-5.4 nano - 최저비용",
    description: "대량 검색, 간단 요약, 빠른 1차 스크리닝에 적합합니다."
  },
  {
    id: "gpt-5.4-mini",
    label: "2. GPT-5.4 mini - 추천 가성비",
    description: "비용과 품질 균형이 좋아 일반 웹 리서치 기본값으로 사용합니다."
  },
  {
    id: "gpt-5.4",
    label: "3. GPT-5.4 - 균형형",
    description: "중요도가 높은 자료 정리나 긴 맥락이 필요한 리서치에 사용합니다."
  },
  {
    id: "gpt-5.5",
    label: "4. GPT-5.5 - 고성능",
    description: "복잡한 추론이 필요한 경우에만 쓰는 프리미엄 옵션입니다."
  }
];

const CHAT_SEARCH_MODEL_OPTIONS = [
  {
    id: DEFAULT_CHAT_SEARCH_MODEL,
    label: `검색 전용: ${DEFAULT_CHAT_SEARCH_MODEL}`,
    description: "Chat Completions 검색 API에서 사용하는 검색 전용 모델입니다."
  }
];

const DEFAULT_MODEL = RESPONSE_MODEL_OPTIONS.some((option) => option.id === process.env.OPENAI_MODEL)
  ? process.env.OPENAI_MODEL
  : "gpt-5.4-mini";

const OPENAI_API_TYPES = [
  {
    id: "responses_web_search",
    label: "Responses API + web_search",
    description: "도메인 필터, 이미지, 라이브 접근 제어, 긴 리서치 토큰 예산을 지원합니다.",
    defaultModel: DEFAULT_MODEL,
    modelOptions: RESPONSE_MODEL_OPTIONS
  },
  {
    id: "chat_completions_search",
    label: "Chat Completions API + gpt-5-search-api",
    description: "기존 Chat Completions 경로입니다. 항상 웹 검색을 실행하지만 일부 웹서치 옵션은 제한됩니다.",
    defaultModel: DEFAULT_CHAT_SEARCH_MODEL,
    modelOptions: CHAT_SEARCH_MODEL_OPTIONS
  }
];
  ...
  properties: {
    ...
    references: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["referenceId", "citation", "url"],
        properties: {
          referenceId: { type: "string" },
...
function buildPrompt({ keyword, selectedTypes, recency, language, memo }) {
  ...
  return [
    ...
    "- Do not reproduce full copyrighted source text. For sourceDossiers.originalExcerpt, include only a short exact excerpt or snippet that is directly visible from search/cited material, keeping it under 25 words per source. Paraphrase everything else.",
    "- For every important source, add one sourceDossiers item that is grouped by source and explains the original evidence, what it supports, and how reliable it is.",
    "- At the very end, add references in a paper-like bibliography style. Use stable referenceIds such as [1], [2], [3], and reuse the same IDs in sourceDossiers.",
...
function getApiType(value) {
  const selected = OPENAI_API_TYPES.find((type) => type.id === value);
  return selected || OPENAI_API_TYPES.find((type) => type.id === DEFAULT_API_TYPE) || OPENAI_API_TYPES[0];
}

function getModelOption(apiType, value) {
  const modelOptions = apiType.modelOptions || RESPONSE_MODEL_OPTIONS;
  const selected = modelOptions.find((option) => option.id === value);
  return selected || modelOptions.find((option) => option.id === apiType.defaultModel) || modelOptions[0];
}

function buildApproximateLocation(body) {
  const location = {
    type: "approximate"
  };
...
function buildOpenAiPayload(body) {
  ...
  const apiType = getApiType(body.apiType);
  ...
  const modelOption = getModelOption(apiType, sanitizeKeyword(body.model));
  ...
  const requestInfo = {
    ...
    allowedDomains,
    blockedDomains,
    externalWebAccess: body.externalWebAccess !== false,
    returnTokenBudget: body.returnTokenBudget || "default"
  };

  if (apiType.id === "chat_completions_search") {
    const webSearchOptions = {};
    if (body.useLocation) {
      const location = buildApproximateLocation(body);
      const approximate = {};
      if (location.country) approximate.country = location.country;
      if (location.city) approximate.city = location.city;
      if (location.region) approximate.region = location.region;
    ...
    return {
      ...
      requestInfo: {
        ...
        returnTokenBudget: "not_supported",
        optionNotice: "Chat Completions 검색 모델은 도메인 필터, 이미지 결과, 라이브 접근 제어, returned-token budget을 지원하지 않습니다."
      }
    };
  }
  ...
  if (body.useLocation) {
    webSearchTool.user_location = buildApproximateLocation(body);
  }
...
async function callOpenAi(payload, requestApiKey, endpointPath = "/v1/responses") {
  ...
  let response;
  try {
    response = await fetch(`https://api.openai.com${endpointPath}`, {
      method: "POST",
      headers: {
...
function collectChatOutputText(response) {
...
function extractChatArtifacts(response) {
  const citations = [];
  ...
  return {
    citations,
    searchActions: [{ type: "chat_completions_web_search" }],
    images: []
  };
}

function normalizeChatCompletionsResponse(response, requestInfo) {
  const outputText = collectChatOutputText(response);
  const structured = tryParseJson(outputText);
  const artifacts = extractChatArtifacts(response);
...
function normalizeOpenAiResponse(response, requestInfo) {
  if (requestInfo?.apiType === "chat_completions_search") {
    return normalizeChatCompletionsResponse(response, requestInfo);
  }
...
async function handleApiConfig(res) {
  json(res, 200, {
    ...
    defaultModel: DEFAULT_MODEL,
    defaultApiType: getApiType(DEFAULT_API_TYPE).id,
